...
$sql="SELECT o.orderid,o.ordertime,o.paystate,o.money,o.ordernum,o.remarks,o.address,o.phone,o.`name`,o.expressPrice,d.price,d.num,gd.dpic,gd.dname,gs.gdname FROM sh_store_order o JOIN sh_store_order_detail d ON o.orderid=d.orderid JOIN sh_store_goods_detail gd ON d.goodsid=gd.detailid JOIN sh_store_goods gs ON gd.gdid=gs.gdid WHERE ordernum ='$orderNum'  AND userid=$userID and o.delstate='N' and o.orderstate='WDELIVER'  ";
...
function OrderCancel(){
	...
	if(cancel_reason.length>75){
		alert("退款原因不能超过75个字！");
		return;	
	}
	$(".show-page-loading-msg").click();
	var params="orderNum="+orderNum+"&reason="+cancel_reason+"&orderid="+orderid+"&m=ORDERBACK";
...
$(document).ready(function(e) {
	var i=0;
    $("#info_detail").hide();
	$("#info_change").click(function(e) {
		//alert(i);
        if(i%2==0){
			$("#info_pic").attr('src','image/hideinfo.png');	
			$("#info_detail").show();
		}else{
			$("#info_pic").attr('src','image/showinfo_detail.png');	
			$("#info_detail").hide();
		}
		i++;
    });	
...
?>
<h1>
  <div style="height:30px; font-size:18px;">申请退款</div></h1>
    ...
    <!--所选商品展示开始-->   
    	...
    	<div id="container"> 
            ...
            <table width="100%" >
               ...
               <tr>
                   <td colspan="1" style="font-size:14px;" >
                    ...
                    <td colspan="1" style="font-size:14px;" >
                  	<?=$dataTable[0]['money']?>小车券  
                   </td>
               </tr>
                <tr>
                   <td  id="info_change" colspan="2" style="font-size:14px; text-align:center;" >
                  	 <img  id="info_pic" src="image/showinfo_detail.png" width="20px" >
                   </td>
               </tr>
           </table> 
           ...
           <input type="hidden" name="orderid" id="orderid"  value="<?=$dataTable[0]['orderid']?>">
          <label for="textarea-enhanced">退款原因:</label>
   			 <textarea name="cancel_reason" id="cancel_reason" style="box-shadow: 0px 1px 3px rgba(0, 0, 0, 0.2) inset;border-radius: 0.3125em;" data-enhanced="true" class="ui-input-text ui-shadow-inset ui-body-inherit ui-corner-all"></textarea>
       
           </div> 
   		
           </br> 
        
     	<a href="#" class="ui-btn ui-corner-all" style=" border-color:#88cc44; color:#88cc44; width:76%; margin:auto;  padding: 0.5em 1em;" onClick="OrderCancel()">提交申请</a>
   		</div> 
        
        <?
		 }else{
		?>
        <p style="margin:30px auto ; text-align:center; color:#F00;">订单号丢失或订单号错误！</p>
        <?
		 }
		?>
    <!--存储地址信息和所提交商品的信息开始--> 
