...
//查询商品信息
$sql="SELECT c.*,d.dname,d.dpic,d.dprice,d.dnum FROM sh_store_cart c JOIN sh_store_goods_detail d ON c.gdetailid =d.detailid WHERE c.state='ON' and checkstate='ON' AND c.userid=".$userID;
//默认地址信息
...
$(function(){
	...
	$("#order_info").click(function(e) {
		//alert(i);
      	if(i%2==0){
			$("#order_detail").fadeIn(300);	
			$("#order_change").attr("src","image/upinfo.png");
		}else{
			$("#order_detail").fadeOut(300);
			$("#order_change").attr("src","image/toinfo.png");
		}
		i++;	  
    });
})
...
function accountit(){
	...
	var params="cartId="+cartId+"&addressId="+addressId+"&remarks="+remarks+"&m=SUBMITORDER"
...
<h1><div style="height:30px; font-size:18px;">填写订单</div></h1>
    ...
    <!--收件地址展示和选择开始-->
		...
		?>
           ...
           <table width="100%" onClick="location.href='ECshopAddressList.php?id=<?=$dataTableAddress[0]['id']?>'">
               <tr>
    ...
    <!--所选商品展示开始-->   
    	<div id="container"> 
           <div style=" width:100%; margin-top:10px; border:#ccc 1px solid" id="order_info">
           <table width="100%" >
               <tr >
                   <td width="93%" style="overflow:hidden;" >
                   <?
				   //把将要提交的购物车id保存到cartId中提交后台处理。
				   	$cartId='';
				   		foreach($dataTable as $items) {
							$cartId=$cartId.$items['id'].",";
				   ?>
                   <div style="float:left; margin-right:10px;">
                   <img src="<?=UPLOADURL.$items['dpic']?>" width="60px"  />
                   <p style="width:60px; margin:0px; text-align:center;" ><?="×".$items['snum']?></p>
                   </div>
                   <?
						}
                   ...
                   <td >
                   		<img src="image/toinfo.png" width="20px;"  id="order_change"  />
                   </td>
               </tr>
		   ...
		   ?>
               <tr style=" margin:8px 0px;  " >
                   ...
                   <td style="padding-top:7px;">
                  		<?=csubstr($infoItems['gsname']."-".$infoItems['gdname'],25,$charset="utf-8")?>
                   </td>
                   <td rowspan="2" style="border-bottom:#ccc 1px solid; color:#F00;">
                   <?=$infoItems['dprice']?>
                   </td>
               </tr>
            ...
            <?
			$summoney=$summoney+($infoItems['snum']*$infoItems['dprice']);
				}
			?>   
            <tr>
                <td colspan="2">
...
<div id="stickey_footer">
  <ul id="footer_menu">
    ...
    <li class="tongji" style=" width:66.24%;">实付款:<span style="font-size:16px;"><?=$summoney+EXPRESSPRICE?>小车券</span></li>
